...
view.Graph = class extends grapher.Graph {
    ...
    // === VQ Phase 3: Highlight API ===
    setHighlightMap(map) {
        this._highlightMap = map || {};
    ...
    _applyHighlights() {
        ...
        const nodes = this._graphElement.querySelectorAll ? this._graphElement.querySelectorAll('.node') : [];
        for (const node of nodes) {
            const nodeId = node.getAttribute('id') || '';
            const name = nodeId.replace(/^node-name-/, '').replace(/^node-id-\d+$/, '');
            let diffType = map[name] || map[nodeId];
            // Fallback for unnamed nodes: match by op_type text content
            if (!diffType && !name) {
                const texts = node.querySelectorAll ? node.querySelectorAll('text') : [];
                for (const text of texts) {
                    const content = text.textContent || '';
                    for (const key of Object.keys(map)) {
                        if (key.startsWith('__op__') && content.includes(key.slice(6))) {
                            diffType = map[key];
                            break;
                        }
                    }
            ...
            const existingBorders = node.querySelectorAll ? node.querySelectorAll('.vq-highlight-border') : [];
            for (const border of existingBorders) {
                border.remove();
            }
            if (diffType && diffColors[diffType]) {
                // 用 getBBox 获取节点实际渲染尺寸
                let nodeWidth = 100;
                let nodeHeight = 50;
                try {
                ...
                border.setAttribute('class', 'vq-highlight-border');
                border.setAttribute('x', String(-padding));
                border.setAttribute('y', String(-padding));
                border.setAttribute('width', String(nodeWidth + padding * 2));
                border.setAttribute('height', String(nodeHeight + padding * 2));
                border.setAttribute('fill', 'none');
                border.setAttribute('stroke', diffColors[diffType]);
    ...
    syncFromExternal(state) {
        ...
        this._scrollTop = state.scrollTop;

        // Update DOM
        canvas.style.width = `${state.zoom * this._width}px`;
        canvas.style.height = `${state.zoom * this._height}px`;
        container.scrollLeft = state.scrollLeft;
        container.scrollTop = state.scrollTop;

        // Notify parent React that sync is complete
        window.parent.postMessage({ type: 'vq-sync-done' }, '*');
    }
    ...
    build(document, origin) {
        if (!origin) {
            ...
            this._graphElement = origin;
        }
        for (const value of this._values.values()) {
            value.build();
        }
        super.build(document, origin);
        // Re-apply highlights after graph is built
        this._applyHighlights();
    }
